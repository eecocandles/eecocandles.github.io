/* script.js
   ...
   - Handles "Order Now" -> opens modal with prefilled mailto: (hello@emberandoak.com)
  ...
  list.forEach(product => {
    ...
    card.innerHTML = `
      <div class="card-media">
        <img src="${product.image}" alt="${product.name} — ${product.scent}" loading="lazy">
      </div>
      <div class="card-body">
        <h4>${product.name}</h4>
...
document.addEventListener('keydown', (e) => { if(e.key === 'Escape' && orderModal.getAttribute('aria-hidden') === 'false') closeModal(); });

contactForm.addEventListener('submit', (e) => { e.preventDefault(); const form = e.target; const formData = new FormData(form); const name = formData.get('name'); const email = formData.get('email'); formResponse.textContent = 'Sending...'; setTimeout(() => { formResponse.textContent = `Thanks ${name || ''}! We received your message and will reply to ${email}.`; form.reset(); setTimeout(() => formResponse.textContent = '', 7000); }, 700); });

const navToggle = document.getElementById('navToggle');
navToggle.addEventListener('click', () => { const nav = document.querySelector('.site-nav'); const expanded = nav.style.display === 'flex'; nav.style.display = expanded ? 'none' : 'flex'; navToggle.setAttribute('aria-expanded', String(!expanded)); });

function observeFadeUps(){ const observer = new IntersectionObserver((entries) => { entries.forEach(entry => { if(entry.isIntersecting){ entry.target.classList.add('in-view'); observer.unobserve(entry.target); } }); }, { threshold: 0.12 }); document.querySelectorAll('.fade-up').forEach(el => { observer.observe(el); }); }
